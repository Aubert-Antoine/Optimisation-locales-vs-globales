...
class AffectOpt{
    public static void main(String[] Args){ int n = Integer.parseInt(Args[0]),
        Hmax = Integer.parseInt(Args[1]); // Juliette peut travailler jusqu'Ã  Hmax heures.
        ...
        afficher(E);        
        // Juliette travaille H heures, 0 à H < Hmax + 1. Affichage des sommes maximum.
        for (int H = 0; H < (Hmax+1); H = H + 1){ // H = nombre d'heures de révision
            System.out.printf("\nNOMBRE D'HEURES TRAVAILLEES : %d\n", H);       
            int[][] E_H = estimationsRestreintes(E,H); // notes estimées pour 0 à h < H+1 
    ...
    // des notes d'une répartition de h heures sur le sous-ensemble des k premières unités.
        ...
        for (int i = 0; i < n; i++) 
            s0 = s0 + E[i][0];
        // Base : m(0,h) = s0 pour tout h, 0 à h < H+1
        for (int h = 0; h < H+1; h++)
            M[0][h] = s0;
        // Cas général, 1 à k < n+1 pour tout h, h, 0 à h < H+1 :
        // m(k,h) = ( Max m(k-1, h - h_k) + e(k-1,h_k) sur h_k, 0 à h_k < h+1 ) - e(k-1,0)
        // Calcul des valeur m(k,h) par k croissants et mémorisation dans le tableau M.
        // Calcul Ã  la volée des a(k,h) = arg m(k,h) et mémorisation dans le tableau A.
        for (int k = 1; k < n+1; k++) // par tailles k croissantes
            for (int h = 0; h < H+1; h++){ // calcul des valeurs m(k,h), 0 à h < H+1
                // Calcul de M[k][h] = 
                // ( Max M[k-1][h-h_k] + e(k-1,h_k), h_k, 0 à h_k < h+1 ) - e(k-1,0)  
                M[k][h] = -1; 
                ...
                // M[k][h] = (max M[k-1][h-h_k] + e(k-1,h_k), h_k, 0 à h_k < h+1)
    ...
    // affiche ro(k,h) : répartition optimale de h heures sur les k premières unités.
        ...
        int akh = A[k][h]; // nombre d'heures allouées Ã  la k-ème unité dans ro(k,h)
